...
class UpdateCategory extends Component {
    constructor(props) {
        ...
        this.handleChangeNameCategory = this.handleChangeNameCategory.bind(this);
        this.handleSubmit = this.handleSubmit.bind(this);
    }

    componentDidMount() {
        var self = this;
        let CategoryId = Number.parseInt(this.props.match.params.categoryId);
        axios.get(`https://localhost:44384/api/ProductCategory?id=${CategoryId}`)
            .then(function (res) {
                if (res) {
                    let cat = { ...res.data.data[0] }
                    self.setState({ CategoryId: cat.ID, NameCat: cat.Name });
                }
            })
    }

    handleChangeNameCategory(e) {
    ...
    handleSubmit(e) {
        let self = this;
        axios({ method: 'put', url: 'https://localhost:44384/api/ProductCategory', data: { ID: self.state.CategoryId, Name: self.state.NameCat } })
            .then(function (res) {
                if (res) {
                    alert("Xửa thành công category");
                }
            })
    }
                                        ...
                                        <fieldset className="ml-4 mt-4">
                                            ...
                                            <div className="form-group">
                                                ...
                                                <div className="col-md-8">
                                                    <input id="product_name" name="product_name" placeholder="PRODUCT NAME" className="form-control input-md" required type="text" value={this.props.NameCart} onChange={this.handleChangeNameCategory} />
                                                </div>
                                            </div>
                                            <div className="form-group">
                                                <div className="col-md-8">
                                                    <button type="Submit" value="Submit" id="singlebutton" name="singlebutton" className="btn btn-primary">UPDATE CATEGORY</button>
                                                </div>
                                            </div>
                                        </fieldset>
                                    </form>
